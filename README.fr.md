...
**Base de connaissances auto-gérée compatible Obsidian pour [pi](https://pi.dev). Suit le modèle LLM Wiki d'Andrej Karpathy.**
...
La plupart des workflows LLM basés sur des fichiers se comportent comme un RAG unique : le modèle recherche des documents bruts à chaque question. La synthèse est éphémère.

**pi-llm-wiki** crée une couche intermédiaire :

- **Les paquets sources bruts préservent les entrées originales**
- **Les pages sources résument ce que chaque source dit**
- **Les pages wiki canoniques suivent ce que le wiki croit actuellement**
- **Les métadonnées générées maintiennent tout accessible par recherche et navigation**

Le résultat est un wiki qui s'accumule au fur et à mesure que vous capturez des sources, posez des questions et archivez des analyses durables.
...
## Architecture de Vault en Couches
...
## Style de Liens et de Citations
