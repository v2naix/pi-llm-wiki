...
4. **Otherwise** (the tool returned extracted content — background unavailable or `background=false`), for each source in the returned batch:
   ...
   b. Do not edit the Source Concept directly; retry controlled background ingestion when available
   ...
   e. Use standard file-relative Markdown links ending in `.md` between related Concepts
   f. Flag any contradictions with existing wiki content using `⚠️ **Contradiction**` markers
5. After processing a synchronous batch, call `wiki_rebuild_meta` to rebuild Private Projections.
...
- Never fabricate information — cite Source Concepts with standard Markdown links such as `[Source](../sources/source.md)`.
